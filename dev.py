...
def generate_team_json(team_id, options):
    ...
    fts = calculate_fts(transfers, next_gw, ru, wc_gws)
    my_data = {
        "chips": chips,
        "picks": [],
        "team_id": team_id,
...
def calculate_fts(transfers, next_gw, ru, wc_gws):
...
def solve_multi_period_fpl(data, options):
    ...
    squad_ru_type_count = {
        (t, w): so.expr_sum(squad_ru[p, w] for p in players if merged_data.loc[p, "element_type"] == t) for t in element_types for w in gameweeks
    }
    ...
    model.add_constraints((so.expr_sum(captain[p, w] for p in players) == 1 + use_dk[w] + 2 * use_aa[w] for w in gameweeks), name="captain_count")
    model.add_constraints((so.expr_sum(vicecap[p, w] for p in players) == 1 - use_dk[w] - use_aa[w] for w in gameweeks), name="vicecap_count")
    model.add_constraints((lineup[p, w] <= squad[p, w] + use_ru[w] for p in players for w in gameweeks), name="lineup_squad_rel")
    model.add_constraints((bench[p, w, o] <= squad[p, w] + use_ru[w] for p in players for w in gameweeks for o in order), name="bench_squad_rel")
    ...
    model.add_constraints(
        (bench[p, w, o] <= squad_ru[p, w] + 1 - use_ru[w] for p in players for w in gameweeks for o in order), name="bench_squad_ru_rel"
    )
    model.add_constraints((captain[p, w] <= lineup[p, w] for p in players for w in gameweeks), name="captain_lineup_rel")
    model.add_constraints((captain[p, w] + use_aa[w] <= 1 for p in players if player_type[p] != 4 for w in gameweeks), name="only_att_allowed_cap_aa")
    ...
    model.add_constraints(
        (so.expr_sum(squad_ru[p, w] for p in players if player_team[p] == t) <= 3 * use_ru[w] for t in teams for w in gameweeks), name="team_limit_ru"
    )
    ...
    # # taken out intentionally because ru has unlimited budget
    # model.add_constraints(
    #     (
    #         so.expr_sum(ru_sell_price[p] * squad[p, w - 1] for p in players) + in_the_bank[w - 1]
    #         >= so.expr_sum(ru_sell_price[p] * squad_ru[p, w] for p in players)
    #         for w in gameweeks
    #     ),
    #     name="ru_budget",
    # )
    model.add_constraints((transfer_in[p, w] <= 1 - use_ru[w] for p in players for w in gameweeks), name="no_tr_in_ru")
    model.add_constraints((transfer_out[p, w] <= 1 - use_ru[w] for p in players for w in gameweeks), name="no_tr_out_ru")

    ## Free transfer constraints
    # 2024-2025 variation: min 1 / max 5 / roll over WC & ru
    raw_gw_ft = {w: free_transfers[w] - transfer_count[w] + 1 - use_wc[w] - use_ru[w] for w in gameweeks}
    ...
    # model.add_constraints((free_transfers[w-1] - number_of_transfers[w-1] - 2 * use_wc[w-1] - 2 * use_ru[w-1] <= 2 * aux[w] for w in gameweeks if w > threshold_gw), name='force_aux_1')
    # model.add_constraints((free_transfers[w-1] - number_of_transfers[w-1] - 2 * use_wc[w-1] - 2 * use_ru[w-1] >= aux[w] + (-14)*(1-aux[w]) for w in gameweeks if w > threshold_gw), name='force_aux_2')
    ...
    if options.get("use_bb", None) is not None:
        ...
        chip_limits["bb"] = 1
    if options.get("use_ru", None) is not None:
        model.add_constraint(use_ru[options["use_ru"]] == 1, name="force_ru")
        chip_limits["ru"] = 1
    ...
    if len(allowed_chip_gws.get("wc", [])) > 0:
        ...
        chip_limits["wc"] = 1
    if len(allowed_chip_gws.get("ru", [])) > 0:
        gws_banned = [w for w in gameweeks if w not in allowed_chip_gws["ru"]]
        model.add_constraints((use_ru[w] == 0 for w in gws_banned), name="banned_ru_gws")
        chip_limits["ru"] = 1
    if len(allowed_chip_gws.get("bb", [])) > 0:
        ...
        chip_limits["bb"] = 1
    if len(allowed_chip_gws.get("dk", [])) > 0:
    ...
    if len(forced_chip_gws.get("wc", [])) > 0:
        ...
        chip_limits["wc"] = 1
    if len(forced_chip_gws.get("ru", [])) > 0:
        model.add_constraint(so.expr_sum(use_ru[w] for w in forced_chip_gws["ru"]) == 1, name="force_ru_gw")
        chip_limits["ru"] = 1
    ...
    if options.get("banned", None):
        ...
        model.add_constraints((so.expr_sum(squad_ru[p, w] for w in gameweeks) == 0 for p in banned_players if p in players), name="ban_player_ru")
    ...
    if options.get("locked", None):
        ...
        model.add_constraints((squad[p, w] + squad_ru[p, w] == 1 for p in locked_players for w in gameweeks), name="lock_player")
    ...
    if max_defs_per_team < 3:  # only add constraints if necessary
        ...
        model.add_constraints(
            (
                so.expr_sum(squad_ru[p, w] for p in players if player_team[p] == t and merged_data.loc[p, "Pos"] in {"G", "D"})
                <= max_defs_per_team * use_ru[w]
                for t in teams
                for w in gameweeks
            ),
            name="defenders_per_team_limit_ru",
        )
    ...
    if options.get("no_gk_rotation_after", None):
        ...
        model.add_constraints(
            (lineup[p, w] >= lineup[p, target_gw] - use_ru[w] for p in players_gk for w in gameweeks if w > target_gw), name="fixed_lineup_gk"
        )

    if len(options.get("no_chip_gws", [])) > 0:
        print("OC - No Chip GWs")
        no_chip_gws = options["no_chip_gws"]
        model.add_constraint(so.expr_sum(use_bb[w] + use_wc[w] + use_ru[w] + use_aa[w] for w in no_chip_gws) == 0, name="no_chip_gws")
    ...
    for iter in range(iteration):
        ...
        for w in gameweeks:
            for p in players:
                if squad[p, w].get_value() + squad_ru[p, w].get_value() + transfer_out[p, w].get_value() > 0.5:
                    lp = merged_data.loc[p]
                    is_captain = 1 if captain[p, w].get_value() > 0.5 else 0
                    is_squad = (
                        1
                        if (use_ru[w].get_value() < 0.5 and squad[p, w].get_value() > 0.5)
                        or (use_ru[w].get_value() > 0.5 and squad_ru[p, w].get_value() > 0.5)
                        else 0
                    )
        ...
        if iteration_criteria == "this_gw_transfer_in":
            ...
            actions = (
                ...
                + so.expr_sum(use_aa[w] for w in gameweeks if use_aa[w].get_value() < 0.5)
            )
            model.add_constraint(actions >= 1, name=f"cutoff_{iter}")
